      ...
      <main style={{ background: '#0A0A0A', minHeight: '100vh' }}>
        ...
        <section className="relative pt-32 pb-16 overflow-hidden">
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <Image src="/images/logo.png" alt="" width={800} height={200}
              className="w-[600px] lg:w-[900px] object-contain select-none"
              style={{ opacity: 0.03 }} aria-hidden />
          </div>
          ...
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            ...
            {featured && (
              <Link href={`/blog/${featured.slug}`} className="group block mb-12">
                <article className="relative rounded-3xl overflow-hidden border transition-all duration-300 hover:-translate-y-1"
                    ...
                    <div className="p-8 lg:p-12 flex flex-col justify-center">
                      <div className="flex items-center gap-3 mb-5">
                        ...
                        <span className="text-xs flex items-center gap-1" style={{ color: '#52525B' }}>
                          <Clock size={11} />{featured.readTime} leitura
                        </span>
                      </div>
                      <h2 className="text-2xl lg:text-3xl font-black tracking-tight mb-4 leading-snug group-hover:text-white transition-colors"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {rest.map(post => (
                <Link href={`/blog/${post.slug}`} key={post.slug} className="group">
                  <article className="h-full rounded-2xl overflow-hidden border transition-all duration-300 hover:-translate-y-1 flex flex-col"
                    style={{ borderColor: 'rgba(255,255,255,0.07)', background: 'rgba(255,255,255,0.02)' }}>
                    ...
                    <div className="p-6 flex flex-col flex-1">
                      <div className="flex items-center gap-2 mb-3">
                        <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full"
                          style={{ background: 'rgba(232,48,48,0.1)', color: '#E83030' }}>
                          {post.category}
                        </span>
                        <span className="text-[10px] flex items-center gap-1" style={{ color: '#52525B' }}>
                          <Clock size={10} />{post.readTime}
                        </span>
                      </div>
                      <h2 className="text-base font-bold mb-2 group-hover:text-[#E83030] transition-colors leading-snug"
                        style={{ fontFamily: "'Satoshi', sans-serif", color: '#F5F5F5' }}>
                        {post.title}
                      </h2>
                      <p className="text-sm leading-relaxed flex-1" style={{ color: '#71717A' }}>{post.excerpt}</p>
                      <div className="flex items-center gap-1.5 mt-4 text-xs font-semibold" style={{ color: '#E83030' }}>
